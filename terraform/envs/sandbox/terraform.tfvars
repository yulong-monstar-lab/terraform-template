tags = {
  "env" : "sandbox",
  "project" : "cicd-test",
}

// Resource group
resource_group_name     = "cicd-test"
resource_group_location = "japaneast"

// webapp module
// Service plan
service_plan_name    = "cicd-test-plan"
service_plan_os_type = "Linux"
service_plan_sku_name = "B1"
# service_plan_sku_name = "F1"


// network module
virtual_network_name              = "Vnet"
virtual_network_address_space     = ["10.0.0.0/16"]
protected_subnet_name             = "protected_subnet1"
protected_subnet_address_prefixes = ["10.0.2.0/24"]
protected_subnet_nsg_name         = "protected_subnet_nsg1"
private_subnet_name               = "private_subnet1"
private_subnet_address_prefixes   = ["10.0.1.0/24"]
private_subnet_nsg_name           = "private_subnet_nsg1"

// dns module
private_dns_zone_name     = "cicd-test.postgres.database.azure.com"
virtual_network_link_name = "virtual_network_link1"

// db
db_name       = "cicd-test-postgresql-server"
db_version    = "12"
db_storage_mb = 32768
db_sku_name   = "B_Standard_B1ms"
db_zone       = "1"
db_high_availability = {
  mode                      = "ZoneRedundant"
  standby_availability_zone = "2"
}
database_name      = "cicd-test-db"
database_collation = "en_US.utf8"
database_charset   = "utf8"

// Web app
web_app_name                        = "nestjs6-web-app"
web_app_https_only                  = true
APP_ENV                             = "development"
APP_PORT                            = "3000"
DB_NAME                             = "cicd-test-db"
DB_PORT                             = "5432"
DB_SSL                              = "true"
JWT_ACCESS_TOKEN_EXP_IN_SEC         = "3600"
JWT_REFRESH_TOKEN_EXP_IN_SEC        = "7200"
WEBSITES_ENABLE_APP_SERVICE_STORAGE = "false"
DEFAULT_ADMIN_USER_PASSWORD         = "cicdtestpwd"
// TODO: Create secretes for below variables in Key Vault
# DB_USER                             = "komatsuadmin"
# DB_PASS                             = "mlazuredb1!"
# JWT_PRIVATE_KEY_BASE64              = ""
# JWT_PUBLIC_KEY_BASE64               = ""

// Container Registry
acr_name = "cicdtestregistry"
acr_sku  = "Basic"

// Key vault
# key_vault_name = "cice-test-yulong"
# key_vault_sku = "standard"

// Use the following command to get key vault id
// az keyvault show --name "cice-test-yulong" | grep "id"
// TODO: Replace key_vault_id with the one you create manually
key_vault_id = "/subscriptions/7420cee8-c116-4416-a874-9d556e6fd400/resourceGroups/cicd-test/providers/Microsoft.KeyVault/vaults/cice-test-yulong"

// Traffic manager
traffic_manager_name    = "cicd-test-manager"
traffic_routing_method  = "Performance"
monitor_config_protocol = "HTTPS"
monitor_config_port     = 443
endpoint_name           = "webapp-endpoint"
# geo_mappings = [""]
